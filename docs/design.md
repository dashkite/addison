...
- **Internal Reactor (`@internal`)**: This reactor manages the model's lifecycle and internal state transitions. It handles logical operations like `resolve`, `valid`, `get`, `put`, and `delete`. By using an internal channel, the model can queue mutations (via the `resolveable` mixin) until it reaches a valid state, ensuring that the model's public interface remains consistent regardless of underlying resource latency.
...
- **`resourceful`**: Defines the basic intent for data interaction (`get`, `put`, `delete`), translating these calls into internal messages for the model's logic.
- **`resolveable`**: Implements the core reactive logic for resource resolution and mutation queuing. It ensures that the model transitions from an "invalid" to a "valid" state and that any pending updates are applied sequentially once the underlying resource is available.
...
- **`Atomic`**: Responsible for managing the lifecycle of a single resource. It defines how raw resource data is transformed into a `Value` instance and how fallback values are applied during "not found" events.
- **`Composite`**: Manages the aggregation of multiple resources. It handles the dynamic creation of sub-resources based on a map of locators and multiplexes their event streams. It is also responsible for determining the overall "valid" state of the aggregate model—typically defined as all sub-resources having reached a valid state.
...
- **`valid`**: A getter that indicates whether the model is ready for interaction.

### Private/Internal Interface

The internal interface facilitates communication between the model's logic and its underlying resources:

- **`_get()`**, **`_put()`**, and **`_delete()`**: Implementation-specific methods for performing the actual resource operations. `Atomic` performs these on a single resource, while `Composite` iterates over its collection of sub-resources.
...
The aggregate state is maintained as a map of sub-resource values. An aggregate `value` event is emitted only when the overall model transitions to a valid state or when one of its sub-resources updates, providing a unified view of the entire resource set to the consumer.
